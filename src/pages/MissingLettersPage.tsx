...
import { useMissingLettersSession } from '../hooks/useMissingLettersSession'
import { MissingLettersCard } from '../components/missingLetters/MissingLettersCard'
...
export function MissingLettersPage() {
  ...
  const highlightOn = currentUser?.highlightModes?.missingLetters ?? false
  const mlWordCount = currentUser?.missingLettersWordCount ?? state.settings.learnWordCount
  const wordCount = mlWordCount === 'all'
    ? activeList?.words.length ?? 6
    : Math.min(mlWordCount, activeList?.words.length ?? 0)
  ...
  const {
    ...
    getFullTyped,
    reset,
  } = useMissingLettersSession(sessionWords, listId)
  ...
  const handleSubmit = useCallback(() => {
    ...
    const fullTyped = getFullTyped()
    const correct = fullTyped === currentWord.word
    if (correct) {
      recordResult(currentWord.word, true)
      checkComplete()
      setShowCelebration(true)
    } else {
      setWrongTyped(fullTyped)
      recordResult(currentWord.word, false)
      setShowEncouragement(true)
    }
  }, [currentWord, getFullTyped, recordResult, checkComplete])
  ...
  if (state.users.length > 0 && !state.currentUserId) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-4">
        ...
        <p className="text-base-content/60">Pick a user from the header to start!</p>
        <motion.button
          whileTap={{ scale: 0.95 }}
