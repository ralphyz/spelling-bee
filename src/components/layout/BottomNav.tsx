...
export function BottomNav() {
  ...
  return (
    <>
      <nav
        className="fixed bottom-0 left-0 right-0 z-40 flex items-stretch bg-base-100/80 backdrop-blur-xl border-t border-base-content/5 h-14"
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              >
                {item.icon}
                <span className={`text-xs leading-none tracking-wide ${active ? 'font-bold' : 'font-medium'}`}>
                  {item.label}
                </span>
              </div>
            </button>
